    ...
    if (enable_glow_hack == 1) {
        if ((int)target->buffer[GLOW_CONTEXT] != 1 || (int)target->buffer[GLOW_VISIBLE_TYPE] != 1 || (int)target->buffer[GLOW_FADE] != 872415232) {
            ...
            if (!isnan(currentEntityTime) && currentEntityTime > 0.f) {
                ...
                Driver::write<Fade>(GamePid, target->ptr + GLOW_FADE, { 1 , 1 , currentEntityTime, currentEntityTime, currentEntityTime, currentEntityTime });
...
void AutoBoneSwitch() {
    ...
    if (nextBoneSwitch < milliseconds_now()) {
        ...
        nextBoneSwitch = milliseconds_now() + 1000;
    }
